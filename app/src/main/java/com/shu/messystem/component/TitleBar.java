...
public class TitleBar extends ViewGroup implements View.OnClickListener {
    ...
    private OnClickListener leftOnClikListener;
    ...
    //读取自定义属性 可通过attrs来获取设置的值
    private void loadCustomAttr(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.TitleBar);
        String title = typedArray.getString(R.styleable.TitleBar_TitleNameCenter);//标题文字
        int titleColor = typedArray.getColor(R.styleable.TitleBar_TitleColor, Color.BLACK); //title文字颜色
        int barBack = typedArray.getColor(R.styleable.TitleBar_BarBackgroundColor, Color.WHITE); //背景默认白色
        int dividerColor = typedArray.getColor(R.styleable.TitleBar_BarDividerColor, Color.GRAY);    //下划线默认灰色
        //  boolean leftImageClick =    typedArray.getBoolean(R.styleable.TitleBar_LeftImageClickFinish, true);  //默认点击左边图片finshactivity
        int leftImage = typedArray.getResourceId(R.styleable.TitleBar_LeftImageResource, R.drawable.nav_back_2x);//默认左边图片 ×
        int rightColor = typedArray.getColor(R.styleable.TitleBar_RightTextColor, getResources().getColor(R.color.font_color2));//默认右边字体颜色
        if (title != null) {
            setTitle(title);
        }

    /*    if (Build.VERSION.SDK_INT>=23||OSUtils.getRomType().equals("MIUI")||OSUtils.getRomType().equals("FLYME")){
            setTitleColor(titleColor);
            setBackgroundColor(barBack);
         }else {//android 6.0以下系统 除(小米和魅族) 不支持状态栏黑色 字体 此时修改titlebar颜色 和主题颜色一致
            setTitleColor(Color.WHITE);
            setBackgroundColor(getResources().getColor(R.color.colorPrimaryDark));
        }*/

        setLeftImageResource(leftImage);
        setActionTextColor(rightColor);
...
/*        setLeftClickListener(new View.OnClickListener() {
        ...
        /*addAction(new TitleBar.TextAction("智慧账户") {
            @Override
        ...
        //    Log.e(attrs.getAttributeName(i),attrs.getAttributeValue(i));
        //}
    }
    ...
    private void initView(Context context) {
        ...
        if(leftOnClikListener!=null){
            mLeftText.setOnClickListener(leftOnClikListener);
        }else{
